...
export const STEPS: StepConfig[] = [
    ...
    fields: [
      {
        ...
        helperText: "You can find your Transaction ID in the payment confirmation email.",
      },
      {
        name: "paymentType",
        label: "Please select the payment for which you are requesting a refund",
        type: "select",
  ...
  },
  {
    ...
    fields: [
      { name: "amountPaid", label: "Amount paid", type: "number", placeholder: "500" },
...
export const STEP_FIELD_NAMES: (keyof RefundFormValues)[][] = STEPS.map((step) =>
  step.fields.map((f) => f.name as keyof RefundFormValues)
);
